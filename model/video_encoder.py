...
def vit_base_patch16_160(pretrained=False, **kwargs):
    ...
    # 加载预训练权重
    pretrained_dict = torch.load("./pretrained_weights/emotion.pth", map_location='cpu')
...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_planes=3, planes=128):
        ...
        self.planes = planes
        self.conv1 = nn.Conv3d(in_planes, planes//4, kernel_size=(5,3,3), stride=(1,2,2), padding=(2,3,3), bias=False)
        self.in1 = nn.InstanceNorm3d(planes//4)
        self.maxpool = nn.MaxPool3d(kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 0, 0))

        self.conv2 = nn.Conv3d(planes//4, planes, kernel_size=(5,3,3), stride=(1,2,2), padding=(2,3,3), bias=False)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, video_feature, emotion_feature):
        """
        :param video_feature: Tensor of shape (B, T, C) 视频特征
        :param emotion_feature: Tensor of shape (B, C) 情感特征
        :return: 加权后的视频特征 Tensor of shape (B, T, C)
        """
        B, T, C = video_feature.size()
        # 将情感特征扩展到时间维度
        emotion_feature_expanded = emotion_feature.unsqueeze(1).expand(-1, T, -1)  # B T C
        
        Q = self.query_transform(video_feature)  # B T C
        K = self.key_transform(emotion_feature_expanded)  # B T C
        V = self.value_transform(emotion_feature_expanded)  # B T C
        
        attention_weights = torch.softmax(torch.bmm(Q, K.transpose(1, 2)) / (K.size(-1) ** 0.5), dim=-1)  # B T T
        attended_emotion = torch.bmm(attention_weights, V)  # B T C
        
        # 将注意力加权后的情感特征与原始视频特征相加
        fused_video_feature = video_feature + attended_emotion
        
        return fused_video_feature
...
class VideoEncoder(nn.Module):
    def __init__(self, img_size=224, feature_dim = 256, device = 'cpu'):
        ...
        self.emotion_model = vit_base_dim512_no_depth_patch16_160()       
    ...
    def forward(self, video):
        ...
        if T>32: 
            video_input_sampled = video_input[:,:, ::32, :, :]   
        elif T>16:
            video_input_sampled = video_input[:,:, ::16, :, :]   
        elif T>8:
            video_input_sampled = video_input[:,:, ::8, :, :]   
        else:
            video_input_sampled = video_input[:,:, ::4, :, :]   
        #video_output = self.Conv3D(video_input)
        emotion_output = self.emotion_model(video_input_sampled)
        #video_output = self.fc(video_output) # B T C
        #fused_video_output = video_output + emotion_output.unsqueeze(1)

        return emotion_output.unsqueeze(1).repeat(1, 256, 1)
